...
Es el núcleo de los datos. Esta capa es una biblioteca de clases estúpida: no tiene lógica, no tiene conexiones, solo tiene la estructura.
...
Es la única capa que tiene permitido saber qué es una base de datos, qué es SQL y cuál es la contraseña de conexión.

\* \*\*Responsable de:\*\* Consultas directas a la base de datos, Operaciones CRUD, mapeo relacional mediante Entity Framework Core.

\* \*\*Interacción con LINQ:\*\* Aquí vive \*\*LINQ to Entities\*\*. Se encarga de traducir las consultas C# a comandos SQL optimizados. Retorna los datos crudos a la capa superior.
